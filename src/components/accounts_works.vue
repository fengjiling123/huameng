<template>
	<div>
		<div class="works-content" id='content'>
			<div v-for='data in accounts_worksdata' class="works" :key='data.id'>
...
export default {
  ...
  mounted(){
  	...
  	this.$store.dispatch('accounts_works',`/api/works?page=1&perPage=12&accountId=${this.$route.params.id}`)
  },
  computed:{
  	...mapGetters(['accounts_worksdata','accounts_workspage','accounts_workstotalpage']),
  	more(){
  		if(this.accounts_workstotalpage==1){
  			return false
  		}
  		return true	
    ...
    methods:{
    	...
    	uppage(){
    		if(this.accounts_workspage==1){
    			...
    			this.nowpage=this.accounts_workspage
    			this.nowpage--
    			this.$store.dispatch('accounts_works',`/api/works?page=${this.nowpage}&perPage=12&accountId=${this.$route.params.id}`)
    			
    		}
    	},
    	downpage(){
    		if(this.accounts_workspage==this.accounts_workstotalpage){
    			//console.log("bb") 
