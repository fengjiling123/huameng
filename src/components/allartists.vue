<template>
	<div>
		<div class="content-artist">
			<div v-for='data,index in artistdata' class="artist-box" :key='data.id'>
				<div class="top clear">
...
export default {
  ...
  mounted(){
  	if(!this.$store.state.allartists){
  		Indicator.open('加载中...')
		this.$store.dispatch('allartists','/api/artists?page=1&perPage=10&official=0')
  	}
  ...
  methods:{
  	uppage(){
  		if(this.artistpage==1){
  			...
  			this.nowpage=this.artistpage
  			this.nowpage--
  			this.$store.dispatch('allartists',`/api/artists?page=${this.nowpage}&perPage=10&official=0`)
  		}
  	},
  	downpage(){
  		if(this.artistpage==this.artisttotalpage){
  			console.log("bb") 
  ...
  computed:{
  	...mapGetters(['artistdata','artistpage','artisttotalpage']),

  	tags(){
  		var tagsarr=[]
  		for(var i=0;i<this.artistdata.length;i++){
  			var temparr=[]
  			if(this.artistdata[i].tags.length<=5){
  				tagsarr.push(this.artistdata[i].tags)
  			}else{
  				for(var j=0;j<5;j++){
  					temparr.push(this.artistdata[i].tags[j])
  				}
